...
import type { BoothConfig, QuoteRequest } from '../types/booth';
...
export default function Step3Preview({ config, onBack, onSubmit }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError('견적 요청 전송에 실패했습니다. 잠시 후 다시 시도해 주세요.');
      setSubmitting(false);
    }
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <Row label="창고" value={elements.hasStorageRoom ? '있음' : '없음'} />
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="행사명" placeholder="2025 서울 국제 박람회" value={form.eventName} onChange={(v) => update({ eventName: v })} />
        ...
        <div className="mt-4">
          ...
          <textarea
            rows={3}
            placeholder="특별 요청사항이 있으면 입력해 주세요"
            value={form.notes}
      ...
      <div className="flex justify-between">
        ...
        <button
          ...
          className="px-8 py-3 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
        >
